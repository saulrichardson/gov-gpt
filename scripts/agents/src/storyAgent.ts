...
function buildStoryInstructions(autonomy: AutonomyMode): string {
  return [
    ...
    "- When using findEndpoints, inspect hasSemanticProfile before calling semantic-only tools such as getAnalysisPacket, getEndpointSemantics, getRequestTemplate, validateRequest, or callEndpoint. If hasSemanticProfile is false, treat that endpoint as a raw-only fallback boundary and do not report it as a promoted semantic route.",
