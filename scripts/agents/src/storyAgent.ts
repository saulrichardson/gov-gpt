...
function buildStoryInstructions(autonomy: AutonomyMode): string {
  return [
    ...
    "- This is an agentic MCP exploration and learning loop, not a deterministic fixture replay.",
    ...
    "- Use the semantic MCP tools as the primary and only promoted query surface.",
    ...
    "- Treat the MCP as a black-box product. Your job is not only to answer the story question; it is to discover reusable semantic learnings that should make future MCP users better.",
    ...
    "- Extract generalizableLearnings from the story. A useful learning explains a reusable pattern such as handoff, measure interpretation, dashboard safety, response shape, request construction, or workflow sequencing. Do not record one-off narrative facts unless they teach a reusable MCP behavior.",
    "- Prefer bundle semantic-affordance repairs before runtime-affordance requests. If meaning exists in prose, joins, workflows, or caveats but not in endpoint.json semanticAffordances, emit a semantic_affordance repair task for the owning bundle.",
    "- Emit runtimeAffordanceRequests only when the bundle already declares the relevant semanticAffordances but the MCP runtime fails to expose them structurally through generic behavior, such as semantic handoff values, measure warnings, recommended follow-ups, or execution receipts.",
    "- Do not ask runtime code to infer endpoint semantics from arbitrary joins, caveats, or prose. The scalable pattern is: agent declares semanticAffordances, runtime applies them generically.",
    ...
    "- Every repair task must include category. Use runtime_affordance only when artifact repairs alone are insufficient; otherwise classify the bundle problem as request_guidance, response_interpretation, workflow_handoff, dashboard_safety, evidence_gap, semantic_affordance, artifact_consistency, or generalization.",
    ...
    "- Use usaspending.getAnalysisPacket before non-trivial requests. Use getEndpointSemantics or getUsageGuide as follow-up when you need the full artifact or prose.",
    "- When using findEndpoints, continue with getAnalysisPacket, getEndpointSemantics, getRequestTemplate, validateRequest, and callEndpoint for the selected promoted semantic slug.",
    ...
    "",
    "Return only the required SemanticStoryReport object, including generalizableLearnings and runtimeAffordanceRequests even when those arrays are empty.",
  ].join("\n");
}
