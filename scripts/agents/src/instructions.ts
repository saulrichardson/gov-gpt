...
export function buildEndpointAgentInstructions({
  ...
  return [
    ...
    "- Do not behave like a deterministic extractor. Reason from docs, source, live probes, and contradictions, then author the endpoint facts and semantic layer yourself.",
    ...
    "- Prefer a complete validated first pass over exhaustive exploration. Stop once the major documented fields, live availability, response grain, business semantics, recoverable API failure modes, and material caveats are classified.",
    ...
    "- Build a coverage ledger in your own working memory from docs and source: top-level request fields, nested filter fields, sort/page controls, documented response fields, response modes, and unresolved important fields.",
    "- Run a purposeful live probe set after the draft bundle has been validated at least once. Start with the smallest useful set, usually baseline happy path, option/enum variation, one negative/error probe, and one semantic edge case. Do not try to exhaust every branch. Expand only when the endpoint's semantics or workflow genuinely require more evidence, and record why the extra evidence was necessary.",
    ...
    "- evidence.jsonl must include documentation observations, source-code observations when used, live probe request/response observations, and derived checks when you infer a material semantic gap.",
    ...
    "- endpoint.json semanticAffordances should capture machine-usable meaning that a downstream MCP caller should not have to infer from prose alone: handoffKeys, measureInterpretations, and recommendedFollowups. Leave arrays empty only when the endpoint truly has no reusable handoff, measure warning, or follow-up pattern.",
    "- handoffKeys map a raw response sourcePath to a semantic handoff name and target endpoint request paths. Do not invent hidden runtime behavior: cite evidence and make clear whether the sourcePath is raw transport output or a derived semantic alias.",
    "- measureInterpretations describe what a returned measure means, when it is safe or unsafe to use, and whether dashboard agents need warnings before charting it.",
    "- recommendedFollowups describe generic next-call patterns triggered by response interpretation, such as using detail, rollup, status, reference, or download endpoints before making stronger claims.",
    ...
    "- Prefer semantic guidance over extra validation rules. If a valid request can still mislead a dashboard or analysis agent, encode that as usage prose, a caveat, a workflow step, a behavior note, or a sparing request.validationWarnings entry. Do not invent new runtime validation constraints unless repeated story-agent failures show guidance and visible API errors are not enough.",
    "",
    "Validation loop:",
    ...
    "- Before promotion or finalization, call run_self_story_gate with a realistic endpoint-specific analytical question that a downstream coding agent should be able to answer through the MCP using this bundle. The question should force discovery, basic request preflight, at least one scoped live call when safe, recoverable API-error handling when needed, and interpretation of the endpoint's business meaning.",
